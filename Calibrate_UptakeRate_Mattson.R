...
#sim <- read_delim("FoodCorrV2_TKV6_Mattson_CalUptakeRate_18DegreesV3", delim = " ")
sim <- read_delim("FoodCorrV2_TKV6_Mattson_CalUptakeRate_26DegreesV3", delim = " ")
...
#sim$SSE_growth <- (sim$InternalConcentration - 1.461111163)^2 # 18 Degrees
sim$SSE_growth <- (sim$InternalConcentration - 1.678944686)^2 # 26 Degrees

# Create unique ID for each iteration/parameter combination
sim$SSEid <- paste(sim$Iteration, sim$UptakeRate, sim$Copper)
...
# Clean and convert UptakeRate (assumed to be column 2)
df$UptakeRate <- as.numeric(as.character(df$X2))

# Compute summary statistics
Assim_cor_max     <- max(df$UptakeRate, na.rm = TRUE)
Assim_cor_median  <- median(df$UptakeRate, na.rm = TRUE)
Assim_cor_Q1      <- quantile(df$UptakeRate, 0.025, na.rm = TRUE)
Assim_cor_Q3      <- quantile(df$UptakeRate, 0.975, na.rm = TRUE)
LL_mean           <- mean(best)
LL_max            <- max(best)
...
# Density plot of UptakeRate for best 100
ggplot(df, aes(x = UptakeRate)) +
