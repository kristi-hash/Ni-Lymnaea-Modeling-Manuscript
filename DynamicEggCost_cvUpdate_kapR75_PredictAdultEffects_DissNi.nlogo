...
to init_globals
  ...
  set kap_R_int                0.75 ;0.5                                        ; -, reproduction efficiency
  ...
  set egg_cost 0.00779 * 0.63 ; cm²d, egg cost in adult-scaled units (= E0_J / p_Am_adult), E0_J = egg_weight × p_Am_base = 0.064941 × 39.42 = 2.56 J, p_Am_adult = t_M × p_Am_base = 8.34 × 39.42 = 328.74 J/d/cm²
...
let e_mother t_v_rate * t_U_E / (t_L ^ 3)
  if e_mother > 1    [ set e_mother 1    ]
  if e_mother < 0.01 [ set e_mother 0.01 ]
  let idx round ((e_mother - 0.01) / 0.01)
  if idx < 0  [ set idx 0  ]
  if idx > 99 [ set idx 99 ]
  let dynamic_U_E0 item idx egg-cost-table
  let dynamic_egg_cost dynamic_U_E0 / t_M

  set t_offspring-number floor ((t_U_R * t_kap_R) / (dynamic_egg_cost / T_fact))

  if t_offspring-number > 0
  [
    set t_repro-time 0
    set t_U_R t_U_R - ((floor ((t_U_R) / (dynamic_egg_cost / T_fact))) * (dynamic_egg_cost / T_fact))
  ...
  if count turtles > 1 ;For adult simulations, eggs are removed and do not have the chance to hatch
...
103
OutputFileName
DynamicEggCost_cvChange_kapEinit75_PredictAdultEffects_Maintenance_DissNiV3
1
0
String
