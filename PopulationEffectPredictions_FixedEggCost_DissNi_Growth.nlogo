...
to setup
  ...
  ;ask turtles [ individual-variability ]
...
to init_globals
  ...
  set kap_R_int                0.475 ;0.5                                        ; -, reproduction efficiency
  ...
  set egg_cost 0.00779 ; cm²d, fixed (non-dynamic) egg cost = E0_J / p_Am_adult, the e=1 (ad libitum) value. E0_J = egg_weight × p_Am_base = 0.064941 × 39.42 = 2.56 J, p_Am_adult = t_M × p_Am_base = 8.34 × 39.42 = 328.74 J/d/cm². Equals the dynamic egg cost at e_mother = 1; matches the older non-population models. (Previously had a × 0.63 factor, removed for a consistent non-dynamic revert.)

  ; Pre-computed lookup table for dynamic egg cost: dynamic_U_E0 for e_m = 0.01 to 1.00 (step 0.01)
  ; Generated from calc-dynamic-U_E0 using Zimmer 2014 / Vlaeminck parameters
  set egg-cost-table [ 0.05614853 0.05614853 0.05614853 0.05614853 0.05614853 0.05614853 0.05614853 0.05614853 0.05614853 0.05614853 0.05614853 0.05614853 0.05614853 0.05614853 0.05614853 0.05614853 0.05614853 0.05614853 0.05614853 0.05614853 0.05614853 0.05614853 0.05614853 0.05614853 0.05614853 0.05614853 0.05614853 0.05615616 0.05617142 0.05618668 0.05621719 0.05625534 0.05630112 0.05633926 0.05640030 0.05645370 0.05653000 0.05659866 0.05666733 0.05675125 0.05683517 0.05691147 0.05701065 0.05708694 0.05720139 0.05729294 0.05739212 0.05749130 0.05759811 0.05771255 0.05782700 0.05794907 0.05803299 0.05817032 0.05830765 0.05840683 0.05855942 0.05865860 0.05877304 0.05893326 0.05905533 0.05916977 0.05929184 0.05942154 0.05958176 0.05971909 0.05985641 0.05998611 0.06011581 0.06026077 0.06042099 0.06058121 0.06074142 0.06082535 0.06100082 0.06117630 0.06127548 0.06145096 0.06162643 0.06173325 0.06193161 0.06203079 0.06223679 0.06234360 0.06253433 0.06267166 0.06279373 0.06301498 0.06312943 0.06327438 0.06348801 0.06361771 0.06373978 0.06393814 0.06412888 0.06426620 0.06439590 0.06453323 0.06473160 0.06493759 ]
  ...
  ; Toxicokinetics (elimination rate) - V3 Kristi
   ...
   set k_u 0.000131304 ; Dissolved nickel temp-corrected rate for 20 degrees dissolved Ni
   ;set k_u 0.000268829 ; Free ion temp-corrected rate for 20 degrees free ion

set volume           15
set ration-amount    9500    ; total joules of food added per feeding (now a total, not a per-litre value)
set feed-days        [ 1 3 5 ]
set waterchange-days [ 1 5 ]
;set mortality-constant random-float 1
end
...
to food-corr
  ...
  [ set food-factor 2.72 ] ;Kristi

  ;if food-type = "Two Week"
  ;[ set food-factor 1.15 ] ;Kristi

  ;if food-type = "Two Week" and day = t_exposure
  ...
  if food-type = "Juvenile" and day >= foodfactor-change and lifestage = "2WFF"
   [ set food-factor 1.93 ]                                     ; J / d cm², maximum surface specific ingestion rat


  if food-type = "Juvenile" and day >= foodfactor-change and lifestage = "2WL"
  [set food-factor 2.32 ] ;Kristi

  if food-type ="AdultTest"
  ;[set food-factor f_min + random-float (f_max - f_min)]
  [set food-factor 1.176]

  if food-type = "AdultTest" and day >= foodfactor-change
  ;[set food-factor f_min + random-float (f_max - f_min)]
  [set food-factor 0.929]                              ; L / d cm², maximum surface specific searching rate
...
to go
  ...
  [
    ...
    if Die? and day >= t_exposure [ death? ]
...
to pop_asses
  ...
  ;let loc_metal (list 0 1.93 3.38 5.97 12.67 30.56 81.99 198.57 0 1.07 3.06 5.72 13.01 31.89 77.39 201.16 0 1.06 2.88 5.31 12.24 29.87 73.72 194.13) ; dissolved Ni juvenile concentrations
  ...
  ;let loc_metal (list 0 0.78 6.24 12.72 30.62 59.80 147.71); dissolved Ni adult concentrations
  let loc_metal (list 0 0.79 2.30 7.13 65.24); diss Ni population experiments
  ;let loc_metal (list 0 0.190 0.409 1.342 12.114); free ion population experiments
...
to-report calc-dynamic-U_E0 [e_m]
  let dyn-embryo-timestep timestep * 50
  let dyn-lower 0
  let dyn-upper 1
  let dyn-sim 0
  let dyn-egg_weight 0
  let dyn-L L_0
  let dyn-U_E 0
  let dyn-U_H 0
  let dyn-e_scaled 0
  ...
  loop
  [
    set dyn-sim dyn-sim + 1
    set dyn-egg_weight 0.5 * (dyn-lower + dyn-upper)
    set dyn-L L_0
    set dyn-U_E dyn-egg_weight
    set dyn-U_H 0
    set dyn-e_scaled v_rate_int * (dyn-U_E / dyn-L ^ 3)
    while [dyn-U_H < U_H^b_int and dyn-e_scaled > e_m]
    [
      set dyn-e_scaled v_rate_int * (dyn-U_E / dyn-L ^ 3)
      set dyn-S_C dyn-L ^ 2 * (g_int * dyn-e_scaled / (g_int + dyn-e_scaled)) * (1 + (dyn-L / (g_int * (v_rate_int / (g_int * k_M_rate_int)))))
      set dyn-U_E dyn-U_E + (-1 * dyn-S_C) / dyn-embryo-timestep
      set dyn-U_H dyn-U_H + ((1 - kap_int) * dyn-S_C - k_J_rate_int * dyn-U_H) / dyn-embryo-timestep
      set dyn-L  dyn-L  + ((1 / 3) * (((v_rate_int / (g_int * dyn-L ^ 2)) * dyn-S_C) - k_M_rate_int * dyn-L)) / dyn-embryo-timestep
      set dyn-e_scaled v_rate_int * (dyn-U_E / dyn-L ^ 3)
    ]
    if (dyn-upper - dyn-lower) < 0.00001
    [ report dyn-egg_weight ]
    ifelse dyn-U_H > U_H^b_int
    [ set dyn-upper dyn-egg_weight ]
    [ set dyn-lower dyn-egg_weight ]
    if dyn-sim > 100
    [ user-message "Dynamic egg cost bisection did not converge." report egg_weight ]
  ]
end

to calc-embryo-reserve-investment
  ; original procedure unchanged from published model
...
to individual-variability
  ...
  set t_K t_p_Xm_rate / F_m * volume ; total food in the tank at half-max
...
to calc-dU_E ; Change in energy reserves
  ...
  [
    ...
    if (Food-dynamics = "Population Experiment")
    [
      if day < t_exposure
      [ask turtles-here
    [set t_functresp f_scaled] ]
...
to lay-eggs
  ...
  ; NON-DYNAMIC egg cost: charge the fixed global egg_cost (set in init_globals),
  ; as in the older non-population models. The maternal-effect lookup is disabled:
  ; (e_mother -> egg-cost-table -> dynamic_egg_cost). The egg-cost-table and the
  ; calc-dynamic-U_E0 reporter remain defined but unused.
  ...
  [
    ...
    set cum_repro cum_repro + (sum [t_offspring-number] of turtles-here)
  ]

  if day = t_exposure [ set cum_repro 0 ] ;only observing reproduction that happens during testing

  if day >= t_exposure ;For population simulations additional individuals are only simulated from the start of the experiment
  [
...
;if day = t_exposure and t_juvenile = 1 [
;  set cv cv_exposure
;  set t_scatter-multiplier e ^ (random-normal 0 cv)
;  set t_p_Xm_rate    t_p_Xm_rate * t_scatter-multiplier
;  set t_p_Am_rate    t_p_Xm_rate * 0.8
;  set t_g            t_g / t_scatter-multiplier]
...
if (Food-dynamics = "Population Experiment") and (day = t_exposure)
[ask patches
  [set p_food ration-amount]]

if (Food-dynamics = "Population Experiment") and (day > t_exposure)
[
  ask patches
  [
    set p_food p_food - p_actualfeeding
    if (ticks mod timestep = 0)
    [
      let dow (day mod 7)
      if member? dow waterchange-days [ set p_food p_food * 0 ]
      if member? dow feed-days        [ set p_food p_food + ration-amount ]
    ]
    if p_food < 0 [ set p_food 0 ]
  ]
]
...
to write_Data_output
  ...
  if (RecordData and ticks mod (timestep) = 0) [
    ...
    file-write cv
    file-write mortality-constant
    file-write ration-amount
    ...
    file-write count turtles with [(t_L / shape_factor * 10) > 3]
    file-write count turtles with [(t_L / shape_factor * 10) > 5]
    file-write count turtles with [(t_L / shape_factor * 10) > 10]
    file-write count turtles with [(t_L / shape_factor * 10) > 15]
    file-write count turtles with [(t_L / shape_factor * 10) > 20]
    file-write count turtles with [(t_L / shape_factor * 10) > 25]
    file-write count turtles with [(t_L / shape_factor * 10) > 30]
...
to write-Heading
  if RecordData [
    ...
    file-write "Count3mm"
    file-write "Count5mm"
    file-write "Count10mm"
    file-write "Count15mm"
    file-write "Count20mm"
    file-write "Count25mm"
    file-write "Count30mm"
...
103
OutputFileName
PopulationEffectlPredictions_Ration9500_JuvMort975_kapR47_FixedEggCosts_DissNi_Growth
1
0
String
...
Food-dynamics
Food-dynamics
"constant" "logistic" "scaled" "intermittent" "Population Experiment"
4
...
Food-type
Food-type
"Lettuce" "Fish flakes" "Periphyton" "Brix" "Freshly Hatched" "Mattson" "calibrate" "Juvenile" "AdultTest"
0
