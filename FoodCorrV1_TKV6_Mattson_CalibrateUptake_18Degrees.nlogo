...
to init_globals
  ...
  ; Toxicokinetics (elimination rate) - V3 Kristi
   ;set k_e k_e_min + random-float (k_e_max - k_e_min) ;calibrating k_e
   set k_e 0.008155186 ;calibrated k_e 18 degrees
   ;set k_e 0.0251 ;calibrated k_e 26 degrees
   ;set k_u k_u_min + random-float (k_u_max - k_u_min) ;calibrating k_e
   ;set k_u  0.00012 ;calibrated k_u 18 degrees
   ;set k_u 0.000228 ;calibrated k_u 26 degrees
   set k_u 0.000247808 ;calibrated k_u 26 degrees free ion
...
to food-corr
  ...
  ;[set food-factor f_min + random-float (f_max - f_min)]; calibrate food-factor
  [ set food-factor 2.42 ]
...
to pop_asses
  ...
  ;let loc_exposure (list 0 45) ;controls only for food calibrations
  ...
  ;let loc_lifestage(list "26C" "26C")
...
to toxicokinetics ; V3 Kristi
 ...
 ;  if day = t_endexposure
   ;[set t_cV 1.461 ;calibrating elimination rate Kristi 18 Degrees
  ;[set t_cV 1.679 ;calibrating elimination rate Kristi 26 Degrees
...
103
OutputFileName
FoodCorrV2_TKV6_Mattson_PredictInternalConcentration_18Degrees_FreeIon
1
0
String
